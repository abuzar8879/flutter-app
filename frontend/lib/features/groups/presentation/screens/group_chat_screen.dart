...
class _GroupChatScreenState extends ConsumerState<GroupChatScreen> {
  ...
  void _onMessageChanged() {
    ref.read(groupChatControllerProvider(widget.group.id).notifier).onMessageChanged();
  }
  ...
  Future<void> _sendText() async {
    ...
    final notifier = ref.read(groupChatControllerProvider(widget.group.id).notifier);
    ...
    try {
      ...
      var message = await socket?.sendGroupMessage(groupId: widget.group.id, type: 'text', content: text);
      message ??= await ref.read(groupsRepositoryProvider).sendMessage(
            token: session.token,
  ...
  Future<void> _pickAndSendImage() async {
    ...
    try {
      ...
      final imagePath = await ref.read(groupsRepositoryProvider).uploadImage(
            token: session.token,
            fileName: file.name,
            bytes: bytes,
          );
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<GroupChatState>(groupChatControllerProvider(widget.group.id), (prev, next) {
...
class _GroupMessageContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (message.type == GroupMessageType.image) {
      final url = message.imageUrl ?? (message.imagePath != null ? '${AppConfig.apiBaseUrl}${message.imagePath}' : '');
      return Column(
